...
export function validateContract(raw: RawOptionContract): ValidationResult {
  ...
  for (const field of CONFIG.REQUIRED_FIELDS) {
    if (
      !(field in raw) ||
      raw[field as keyof RawOptionContract] === null ||
      raw[field as keyof RawOptionContract] === undefined
    ) {
  ...
  try {
    ...
    if (isNaN(vol) || vol < 0) {
      warnings.push(`Invalid volume: ${raw.vol}, setting to 0`);
    }
    ...
    if (!isValidDate(raw.date)) {
      errors.push(`Invalid date format: ${raw.date}`);
    }

    if (!isValidDate(raw.expiration)) {
      errors.push(`Invalid expiration format: ${raw.expiration}`);
    }
    ...
    const cleanedData: ProcessedOptionContract = {
      ...
      strike: +strike.toFixed(CONFIG.DECIMAL_PRECISION),
      type: normalizeCallPutType(raw.call_put as CallPutType),
      bid: +bid.toFixed(CONFIG.DECIMAL_PRECISION),
      ask: +ask.toFixed(CONFIG.DECIMAL_PRECISION),
      volume: Math.max(0, vol || 0),
      delta: +(+(raw.delta || 0).toFixed(CONFIG.DECIMAL_PRECISION)),
      gamma: +(+(raw.gamma || 0).toFixed(CONFIG.DECIMAL_PRECISION)),
      theta: +(+(raw.theta || 0).toFixed(CONFIG.DECIMAL_PRECISION)),
      vega: +(+(raw.vega || 0).toFixed(CONFIG.DECIMAL_PRECISION)),
      rho: +(+(raw.rho || 0).toFixed(CONFIG.DECIMAL_PRECISION)),
    };
...
export async function writeJsonFile(
  ...
  try {
    ...
    if (buffer.length > CONFIG.MAX_FILE_SIZE) {
      throw new Error(
        `File too large: ${buffer.length} bytes > ${CONFIG.MAX_FILE_SIZE} bytes`,
      );
    }
...
export function groupBySymbol(
  ...
  for (const contract of contracts) {
    ...
    grouped.get(symbol)!.push(contract);
  }

  // Sort contracts within each symbol group
...
export function logPerformanceMetrics(metrics: PerformanceMetrics): void {
  const memoryFreed =
    metrics.memoryBefore.heapUsed - metrics.memoryAfter.heapUsed;
  ...
  console.log(
    `   Memory Used: ${formatFileSize(metrics.memoryAfter.heapUsed)}`,
  );
